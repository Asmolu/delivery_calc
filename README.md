...
2. **Frontend** — Vite dev-server c пробросом API на localhost:
   ...
   - Интерфейс теперь в тёмной теме: карточки и таблицы «под Яндекс», адаптив для ПК и мобильных.

## 🐳 Docker/Compose
Требуются `.env` и `google_credentials.json` рядом с `docker-compose.yml`. Полный цикл сборки и запуска:
```bash
