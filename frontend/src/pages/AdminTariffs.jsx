...
import { adminDeleteTransportCard, adminListTariffAudit, adminListTariffs, adminUpsertTransportCard } from "../api";
...
export default function AdminTariffs() {
  ...
  const loadAudit = async () => {
    ...
    try {
      ...
      setMessage(e?.message || "Ошибка загрузки истории изменений");
    } finally {
  ...
  const saveCard = async () => {
    ...
    try {
      ...
      const blocks = (draft.blocks || []).slice(0, 2).map((b) => {
        const cond = b.weight_condition || "any";
        const thr = cond === "any" ? null : toNum(b.weight_threshold || 0, 0);
        if (cond !== "any" && !thr) throw new Error("Weight threshold обязателен для le/gt");
      ...
      const payload = {
        ...
        enable_delivery: !!draft.enable_delivery,
        enable_unloading: !!draft.enable_unloading,
  ...
  const deleteCard = async () => {
    ...
    setMessage("");
    try {
      await adminDeleteTransportCard(selectedTransport.name, selectedTransport.tag);
      setSelectedKey("new");
      await reload();
    } catch (e) {
    ...
    <MotionDiv className="space-y-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="card-glass p-6 border border-slate-200 bg-white">
          ...
          <label className="text-sm md:col-span-1">
            ...
            <div className="flex flex-col gap-2 pt-1">
              <label className="flex items-center gap-2 text-sm text-slate-700">
                <input type="checkbox" checked={!!draft.enable_delivery} onChange={(e) => setDraft((s) => ({ ...s, enable_delivery: e.target.checked }))} className="w-4 h-4 accent-indigo-600" />
                Доставка
              </label>
              <label className="flex items-center gap-2 text-sm text-slate-700">
                <input type="checkbox" checked={!!draft.enable_unloading} onChange={(e) => setDraft((s) => ({ ...s, enable_unloading: e.target.checked }))} className="w-4 h-4 accent-indigo-600" />
                Разгрузка
              </label>
            </div>
          </label>
          ...
          <label className="text-sm md:col-span-1">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setDraft((s) => ({ ...s, unload_tags: selected }));
              }}
              className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white min-h-[3.25rem]"
            >
              <option value="self">Сам</option>
              <option value="crane">Кран</option>
              <option value="manipulator">Манипулятор</option>
            </select>
            <div className="text-xs text-slate-500 mt-1">Зажмите Ctrl/Shift для множественного выбора.</div>
          </label>
        ...
        <div className="mt-5 space-y-5">
          {draft.blocks.slice(0, 2).map((b, bi) => (
            <div key={bi} className="border border-slate-200 rounded-xl p-4 bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <label className="text-sm">
                  <div className="text-slate-600 mb-1">Weight threshold (т)</div>
                  <input
                    type="number"
                    step="0.1"
        ...
        {auditOpen ? (
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={loadAudit} disabled={auditLoading} className="px-3 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-500 disabled:opacity-60">
                Обновить историю
              </button>
            </div>
